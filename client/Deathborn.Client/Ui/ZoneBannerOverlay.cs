...
public sealed class ZoneBannerOverlay
{
    ...
    private void DrawPersistentPill(SpriteBatch sb, SpriteFont font)
    {
        ...
        SpriteFontSafe.DrawString(sb, font, label, new Vector2(rect.X + 20, rect.Y + 5), new Color(200, 255, 210));
    }
        ...
        TinySwordsUi.RibbonKind ribbonKind, bool pointed, float fade)
    {
        ...
        SpriteFontSafe.DrawString(sb, font, title, new Vector2(textArea.X, textArea.Y + 4),
            new Color(255, 248, 220) * fade, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);

        if (!string.IsNullOrEmpty(subtitle))
        {
            SpriteFontSafe.DrawString(sb, font, subtitle,
    ...
    private void DrawLegacy(SpriteBatch sb, SpriteFont font)
    {
        if (InSafeZone && !string.IsNullOrEmpty(PersistentZoneName))
        {
            ...
            SpriteFontSafe.DrawString(sb, font, label, new Vector2(rect.X + 10, rect.Y + 4), new Color(140, 220, 160));
        }

        if (!_active || _fade <= 0.01f) return;

        var titleSize = SpriteFontSafe.MeasureString(font, _title);
        var subSize = SpriteFontSafe.MeasureString(font, _subtitle);
        ...
        SpriteFontSafe.DrawString(sb, font, _title, new Vector2(panel.Center.X - titleSize.X / 2f, panel.Y + 10), new Color(240, 225, 180) * _fade);
        SpriteFontSafe.DrawString(sb, font, _subtitle, new Vector2(panel.Center.X - subSize.X / 2f, panel.Y + 10 + font.LineSpacing + 2),
