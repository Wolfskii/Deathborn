...
public sealed class WorldBackgroundRenderer
{
    ...
    public void Draw(SpriteBatch sb, Vector2 camera, Vector2 screenCenter)
    {
        Vector2 ToScreen(Vector2 w) => w - camera + screenCenter;

        var cols = (int)(HalfExtent * 2 / Cell) + 1;
        var start = new Vector2(-HalfExtent, -HalfExtent);

        var groundScreen = ToScreen(new Vector2(-HalfExtent, -HalfExtent));
        DrawPrimitives.FillRect(sb,
            new Rectangle((int)groundScreen.X, (int)groundScreen.Y, (int)(HalfExtent * 2), (int)(HalfExtent * 2)), Ground);
        ...
        for (var col = 0; col < cols; col++)
        {
            ...
            var rect = new Rectangle((int)screen.X, (int)screen.Y, (int)Cell, (int)Cell);
        ...
        for (var i = 0; i <= cols; i++)
        {
            ...
            DrawPrimitives.FillRect(sb, new Rectangle(xScreen, (int)ToScreen(new Vector2(xWorld, -HalfExtent)).Y, 1, (int)(HalfExtent * 2)), Grid);
            DrawPrimitives.FillRect(sb, new Rectangle((int)ToScreen(new Vector2(-HalfExtent, yWorld)).X, yScreen, (int)(HalfExtent * 2), 1), Grid);
        }

        var origin = ToScreen(Vector2.Zero);
        DrawPrimitives.DrawLine(sb, origin + new Vector2(-24, 0), origin + new Vector2(24, 0), Origin, 2);
        DrawPrimitives.DrawLine(sb, origin + new Vector2(0, -24), origin + new Vector2(0, 24), Origin, 2);
        DrawPrimitives.DrawCircleOutline(sb, origin, 8, Origin);
    }
}
